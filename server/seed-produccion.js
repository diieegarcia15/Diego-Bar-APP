...
if (prodsExistentes.count === 0) {
  ...
  const beer = 'https://images.unsplash.com/photo-1535958636474-b021ee887b13?w=800';
  const gin = 'https://images.unsplash.com/photo-1551538827-9c037cb4f32a?w=800';
  const papas = 'https://images.unsplash.com/photo-1573080496219-bb080dd4f877?w=800';
  const pizza = 'https://images.unsplash.com/photo-1513104890138-7c749659a591?w=800';
  const burger = 'https://images.unsplash.com/photo-1568901346375-23c9450c58cd?w=800';
  const mila = 'https://images.unsplash.com/photo-1632778149955-e80f8ceca2e8?w=800';
  const sandwich = 'https://images.unsplash.com/photo-1553909489-cd47e0907980?w=800';
  const ensalada = 'https://images.unsplash.com/photo-1512621776951-a57141f2eefd?w=800';
  const pasta = 'https://images.unsplash.com/photo-1587740908075-9e245070dfaa?w=800';
  const postre = 'https://images.unsplash.com/photo-1624353365286-3f8d62daad51?w=800';
  const bebida = 'https://images.unsplash.com/photo-1622483767028-3f66f32aef97?w=800';
  const trago = 'https://images.unsplash.com/photo-1510626176961-4b57d4fbadf3?w=800';
  const nachos = 'https://images.unsplash.com/photo-1513456852971-30c0b8199d4d?w=800';
  ...
  const run = db.transaction(() => [
    ...
    ['ROBUST PORTER','Negra, cafe y chocolate.',5500,beer,1],
    ...
    ['WEE HEAVY KM 213','Corpulenta y dulce.',6400,trago,1],
    // Happy Hour (2)
    ...
    ['PULLED MEAT EXTREMAS','Carne desmechada y queso.',24000,papas,4],
    // Entrantes (5)
    ['NACHOS CON QUESO','Triangulos de maiz.',12000,nachos,5],
    ['TEQUENOS x6','Masa rellena de queso.',14000,null,5],
    ['BASTONCITOS DE MUZZARELLA','Fritos con salsa pomodoro.',15000,pizza,5],
    ['RABAS A LA ROMANA','Anillos de calamar.',21000,null,5],
    ['LANGOSTINOS REBOZADOS','Con salsa alioli.',19000,null,5],
    // Pizzas (6)
    ...
    ['NAPOLITANA MIL','Jamon y muzzarella.',22000,pizza,8],
    ...
    // Wraps (10)
    ['WRAP DE POLLO','Con vegetales.',15000,ensalada,10],
    ['WRAP VEGGIE','Con humus.',14000,ensalada,10],
    ...
    // Menu Infantil (12)
    ['MILANESITA CON PAPAS','Porcion infantil.',12000,papas,12],
    ['HAMBURGUESITA','Con queso.',11000,burger,12],
    // Sin TACC (13)
    ['PIZZA SIN TACC','Apta celiacos.',23000,pizza,13],
    ['LOMITO SIN TACC','Apto celiacos.',21000,trago,13],
    // Postres (14)
    ['VOLCAN DE CHOCOLATE','Con helado.',8500,postre,14],
    ['CHOCOTORTA','En frasco.',7500,postre,14],
    // Bebidas (15)
    ...
    // Tragos clasicos (16)
    ['FERNET BRANCA','Con Coca.',8500,trago,16],
    ['CAMPARI TONIC','Con naranja o tonica.',8000,trago,16],
    ['NEGRONI','Gin, vermut y campari.',9000,trago,16],
    // Caribenos (17)
    ['MOJITO','Ron y menta.',9500,trago,17],
    ['CAIPIROSKA','Vodka y lima.',9000,trago,17],
    ['MARGARITA','Tequila y triple sec.',9500,trago,17],
    // Frozens (18)
    ['DAIQUIRI FROZEN','Frutilla o durazno.',9500,trago,18],
    ['MARGARITA FROZEN','Limon o frutilla.',9800,trago,18],
  ].forEach(args => ins.run(...args)));
  run();
  console.log('57 productos insertados');
}
